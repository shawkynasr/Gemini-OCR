...
class RequestWorker(QObject):
    ...
    def parse_paddle_response(self, data):
        """Extracts text based on the exact PaddleOCR-VL-1.5 JSON spec"""
    ...
    def send_baidu_request(self, image_data):
        file_b64 = base64.b64encode(image_data).decode('ascii')
        headers = {
                        ...
                        for attempt in range(MAX_RETRIES + 1):
                            try:
                                if self.engine == "Google Gemini":
                                    ...
                                    response = self.send_baidu_request(image_data)
                                    ...
                                    text_response = self.parse_paddle_response(response.json())
                                    break
...
6.  **Clean Output:** Return **ONLY the extracted text** without any explanations, introductions, or markdown code blocks tags."""
        ...
        url_label = QLabel("🔗 Custom API URL:")
        ...
        left_column_layout.addWidget(self.url_container)
        ...
        self.baidu_prompt_label = QLabel("🤖 Baidu Task:")
        self.baidu_prompt_combo = QComboBox()
        self.baidu_prompt_combo.addItems(["ocr", "table", "formula", "chart"])
        
        model_layout.addWidget(self.model_label)
        model_layout.addWidget(self.model_combo)
        model_layout.addWidget(self.baidu_prompt_label)
        model_layout.addWidget(self.baidu_prompt_combo)
        ...
        # ADD STRETCH FACTOR TO PREVENT THE UI GAP BUG ON MACOS!
    ...
    def toggle_engine_ui(self):
        ...
        # NEVER HIDE THE BOXES. JUST ENABLE / DISABLE THEM TO PREVENT UI JUMPING!
        self.custom_url_input.setEnabled(not is_gemini)
        self.proxy_input.setEnabled(is_gemini)
        
        # Disable prompt and its toggle button when Baidu is active
        self.prompt_input.setEnabled(is_gemini)
        self.toggle_prompt_btn.setEnabled(is_gemini)
        
        self.model_combo.setEnabled(is_gemini)
        self.baidu_prompt_combo.setEnabled(not is_gemini)

        # Update Batch Tooltip/Visibility
        self.image_batch_spin.setEnabled(is_gemini)
        if not is_gemini:
